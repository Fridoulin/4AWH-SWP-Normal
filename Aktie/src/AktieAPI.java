...
public class AktieAPISQL extends Application{
    ...
    public static void main (String args[]) throws Exception {
        ...
        a.connectToMySql();
        a.inputUser();
        for(int i = 0; i < auswahlAktie.size(); i++) {
            String tempAktie = auswahlAktie.get(i);
            a.clear();
            a.readURL(tempAktie);
            a.selectToCheck(tempAktie);
            a.createTable(tempAktie);
            a.getWert(URL);
            a.writeDataInDB(tempAktie);
            a.durchschnitt(tempAktie);
            a.getData(tempAktie);
        }
            Application.launch(args);
    }
    ...
    public static void selectToCheck(String tempAktie) {
        try {
            myStmt = connection.createStatement();
    ...
    static boolean createTable(String tempAktie) throws SQLException {
        try{
            ...
            String createtableAVG = "create table if not exists "+tempAktie+"AVG (datumAVG varchar(255) primary key, avg double);";
            myStmt.executeUpdate(createtable);
            myStmt.executeUpdate(createtableAVG);
            return true;
        } catch (SQLException e) {
    ...
    void durchschnitt(String tempAktie) {
        try {
        ...
        try {
            myStmt = connection.createStatement();
            for(int i = 0; i < daten.size(); i++) {
                String writeData = "insert ignore into "+tempAktie+"AVG (datumAVG, avg) values('"+daten.get(i)+"', '" + gleitenderDurchschnitt.get(i) + "');";
                myStmt.executeUpdate(writeData);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static void getData(String tempAktie) {
        try {
            ...
            ResultSet rsNormal = myStmt.executeQuery( "SELECT * from " + tempAktie + " order by datum");
            ResultSet rsAVG = myStmtAVG.executeQuery("SELECT * from " + tempAktie+"AVG order by datumAVG");
    ...
    public void start(Stage primaryStage) {
        try {
            ...
            for(int i = 0; i < auswahlAktie.size(); i++) {
                lineChart.setTitle("Aktienkurs " + auswahlAktie.get(i));
            }
            ...
            for(int i = 0; i < auswahlAktie.size(); i++) {
                ...
                File file = new File("C:\\Users\\nisch\\IdeaProjects\\AktieAPISQL\\Image\\" + newFolder + "\\" + auswahlAktie.get(i) + " " + LocalDate.now() + ".png"); //Pfad einfügen
                ImageIO.write(SwingFXUtils.fromFXImage(image, null), "PNG", file);
                System.out.println("Image Saved "+auswahlAktie.get(i));
            }
        } catch(Exception e) {
            e.printStackTrace();
        }
